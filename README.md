...
This repo dogfoods its own config: **[`crap.toml`](crap.toml)** at the root is a real, trimmed-down working example (the kind you'd actually commit, not the exhaustive dump) — it sets the `preset`, `metric`, multi-root `src` array, and a `[views.ci]` preset that the production CRAP scorecard CI job consumes via auto-discovery, with no analysis flags in the workflow. Read it alongside `crap.example.toml` to see the difference between "every option documented" and "a real project's config."
